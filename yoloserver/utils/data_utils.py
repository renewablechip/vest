...
from yoloserver.utils.paths import YOLO_STAGED_LABELS_DIR
from yoloserver.utils.data_converters.coco import convert_coco_json_to_yolo
from yoloserver.utils.data_converters.pascal_voc import convert_pascal_voc_to_yolo
...
def convert_annotations_to_yolo(input_dir: Path,
    ...
    try:
        if annotation_format == "coco":
            ...
            if not classes:
                logger.error(f"转换Pascal VOC格式的标注数据时失败,未提取到任何类别")
                return []
