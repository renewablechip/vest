...
# --- 新增美化和项目级配置的默认值 ---
    ...
    'font_size': 26,             # 注意这里是 font_size, 与命令行参数名对应
    'line_width': 4,             # 注意这里是 line_width, 与命令行参数名对应
    'label_padding_x': 10,
    'label_padding_y': 10,
    'radius': 8,
    'display_size': 720,
    'text_color': [0, 0, 0], # 默认文本颜色（BGR，黑色）
    'font_path_english':"fonts/arial.ttf",     # 默认英文字体
    'font_path_chinese':"fonts/msyh.ttc",     # 默认中文字体 (微软雅黑)
    'label_mapping' : {
        'no-vest': '未穿戴',
        'vest': '已穿戴'
    },
    'color_mapping' : {
    'no-vest': [255, 0, 0], # 改为列表
    'vest': [0, 255, 0]      # 改为列表
    },
...
COMMENTED_INFER_CONFIG = (
    ...
    f"beautify: {DEFAULT_INFER_CONFIG['beautify']}\n"
    "# (*)启用中文显示，适合高质量可视化，默认 True\n"
    f"use_chinese_mapping: {DEFAULT_INFER_CONFIG['use_chinese_mapping']}\n"
    "# (*)美化字体大小，以 720p 分辨率为基准，自动缩放，默认 22\n"
    f"font_size: {DEFAULT_INFER_CONFIG['font_size']}\n"
    "# (*)美化线宽，用于绘制检测框和标签，以 720p 为基准，自动缩放，默认 4\n"
    f"line_width: {DEFAULT_INFER_CONFIG['line_width']}\n"
    "# (*)实时显示窗口的短边尺寸 (px)，用于动态计算字体、线宽\n"
    f"display_size: {DEFAULT_INFER_CONFIG['display_size']}\n"
    "# (*)美化标签水平内边距，以 720p 为基准，自动缩放，默认 30\n"
    f"label_padding_x: {DEFAULT_INFER_CONFIG['label_padding_x']}\n"
    "# (*)美化标签垂直内边距，以 720p 为基准，自动缩放，默认 18\n"
    f"label_padding_y: {DEFAULT_INFER_CONFIG['label_padding_y']}\n"
    "# (*)美化圆角半径，用于标签圆角效果，以 720p 为基准，自动缩放，默认 8\n"
    f"radius: {DEFAULT_INFER_CONFIG['radius']}\n"
    "# (*)默认文本颜色\n"
    f"text_color: {DEFAULT_INFER_CONFIG['text_color']}\n"
    "# (*)默认英文路径\n"
    f"font_path_english: {DEFAULT_INFER_CONFIG['font_path_english']}\n"
    "# (*)默认中文路径\n"
    f"font_path_chinese: {DEFAULT_INFER_CONFIG['font_path_chinese']}\n"
    "# (*)标签中文映射\n"
    f"label_mapping: {DEFAULT_INFER_CONFIG['label_mapping']}\n"
    "# (*)标签框颜色映射\n"
    f"color_mapping: {DEFAULT_INFER_CONFIG['color_mapping']}\n"
    "# (*)参考分辨率（用于字体大小和线宽的缩放基准）\n"
    f"ref_dim_for_scaling: {DEFAULT_INFER_CONFIG['ref_dim_for_scaling']}\n"
    "# (*)支持的分辨率映射，用于计算运行时参数\n"
    f"resolution_dims: {DEFAULT_INFER_CONFIG['resolution_dims']}\n"
