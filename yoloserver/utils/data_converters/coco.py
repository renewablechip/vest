...
from yoloserver.utils.paths import RAW_DATA_DIR, YOLO_STAGED_LABELS_DIR
...
def convert_coco_json_to_yolo(json_input_dir: Path, task: str = "detection",cls91to80: bool = False):
    ...
    try:
        _ = convert_coco(
            labels_dir=str(json_input_dir), # 输入的coco json所在目录
            save_dir = str(temp_dir), # 保存转换后的数据到临时目录
            use_segments=use_segments, # 是否使用分割标注
            use_keypoints=False, # 目前不处理关键点数据
            cls91to80=cls91to80, # 是否将coco 91类映射到80类
            lvis=False # lvis参数在这里不需要设置为True，因为我们只处理coco数据集
        )
...
if __name__=="__main__":
    classes_name_ = convert_coco_json_to_yolo(
        json_input_dir=Path(__file__).resolve().parent.parent.parent / "data" / "raw" / "original_annotations",
        task = "detection",
    )
    print(classes_name_)