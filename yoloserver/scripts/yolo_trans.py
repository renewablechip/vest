...
from yoloserver.utils.data_utils import convert_annotations_to_yolo
...
class YOLODatasetProcessor:
    ...
    3. 生成data.yaml配置文件
    """
    def __init__(self,train_rate=0.9,valid_rate=0.05,annotation_format="coco",
                coco_task="detection",
    ...
    def _process_single_split(self, label_files, image_files, split_name):
        ...
        :param label_files: YOLO TXT标签文件列表
    ...
    def process_dataset(self,source_data_root_dir=ORIGINAL_ANNOTATIONS_DIR,):
        """
        执行整个数据集划分流程
        :param source_data_root_dir: 原始数据集的根目录，包含图像和标注文件
        :return:
        """
        ...
        try:
            ...
            if self.annotation_format == "yolo":
                if not self.classes:
                    logger.critical(f"当 annotation_format 为 yolo 时，请务必提供 classes 参数，数据集处理终止")
                    return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_rate", type=float, default=0.9, help="训练集占比,默认0.8")
    parser.add_argument("--valid_rate", type=float, default=0.05, help="验证集占比,默认0.1")
