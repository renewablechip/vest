...
from yoloserver.utils import setup_logger, time_it
from yoloserver.utils.paths import LOGS_DIR, CHECKPOINTS_DIR, PRETRAINED_DIR
...
from yoloserver.utils.logging_utils import rename_log_file
...
from yoloserver.utils.logging_utils import log_parameters
def parse_args():
    """命名行解析参数"""
    ...
    parser.add_argument("--weights", type=str, default= "last.pt",help="预训练模型路径")
    ...
    parser.add_argument("--resume", action='store_true', help="从最新的检查点继续训练")
...
def main(args, logger):
    logger.info("YOLO 背心检测训练脚本启动".center(80, "="))
    try:
        yaml_config = {}
        if args.use_yaml:
